...
public class App {
    ...
    public static int calculateMiddlePages() {
        ...
        while (inputReader.hasNext()) {
            ...
            if (!updateObeysOrder(updateNumbers)) {
                LinkedList newList = createNewList(updateNumbers);
                while (!updateObeysOrder(newList)) {
                    newList = createNewList(newList);
                }
                sum += getMiddlePage(newList);
            }
        }
    ...
    public static LinkedList createNewList(LinkedList<Integer> update) {
        LinkedList<Integer> alreadyProcessedNumbers = new LinkedList<>();
        ...
        for (Integer processingNumber : update) {
            if (orderingRules.containsKey(processingNumber)) {
                ...
                if (numberShouldHaveBeenProcessedLater(alreadyProcessedNumbers, numbersAfter)) {
                    int removeLast = alreadyProcessedNumbers.removeLast();
                    alreadyProcessedNumbers.add(processingNumber);
                    alreadyProcessedNumbers.add(removeLast);
                } else {
                    alreadyProcessedNumbers.add(processingNumber);
                }
        ...
        return alreadyProcessedNumbers;
    }
}
